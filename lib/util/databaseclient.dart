...
import 'package:StickyNote/modal/notepaditem.dart';
...
//insertion
  Future<int> saveItem(NotepadItem item) async {
...
//
  Future<NotepadItem> getItem(int id) async {
    var dbClient = await db;
    var result =
    await dbClient.rawQuery("SELECT * FROM $tableName WHERE id = $id");
    if (result.length == 0) return null;
    return new NotepadItem.fromMap(result.first);
  }
...
//  }
  ...
  Future<int> updateItem(NotepadItem item) async {
    var dbClient = await db;
